...
/// let constraints = Constraint::from_proportionals([1, 2, 1]);
/// ```
...
pub enum Constraint {
    ...
    ///
    /// The element size is set to the specified amount as
    ...
    ///
    /// The element size is set to the given amount as
    /// a strong preference.
    ///
    /// # Examples
    ///
...
impl Constraint {
    ...
    /// let layout = Layout::default().constraints(constraints).split(area);
    /// ```
    pub fn from_proportionals<T>(proportionals: T) -> Vec<Constraint>
    where
        T: IntoIterator<Item = u16>,
    {
        proportionals
...
mod tests {
    ...
    #[test]
    fn from_proportionals() {
        let expected = [
            Constraint::Proportional(1),
            Constraint::Proportional(2),
            Constraint::Proportional(3),
        ];
        assert_eq!(Constraint::from_proportionals([1, 2, 3]), expected);
        assert_eq!(Constraint::from_proportionals(vec![1, 2, 3]), expected);
    }
